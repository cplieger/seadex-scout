...
// FuzzBoundedValidator fuzzes the persisted-validator sanitizer against
// arbitrary header bytes from the hostile upstream (response headers) and the
// tamperable state file (persisted validators). Invariants: the result is
// either empty or the input unchanged (all-or-nothing, never a partial
// rewrite); a non-empty result fits maxValidatorBytes and carries only RFC
// 9110 field-value-legal bytes (no control characters other than tab, no
// DEL), so a survivor can never be rejected by net/http at request-write time
// when replayed as a conditional-GET header; and the sanitizer is idempotent,
// so a validator that survived the accept-200 sanitization is never dropped
// by the load-side pass on the next cycle.
func FuzzBoundedValidator(f *testing.F) {
	f.Add("W/\"abc-123\"")
	f.Add("\"etag\r\nX-Injected: 1\"")
	f.Add("\"et\x01ag\"")
	f.Add("\"et\x7fag\"")
	...
	f.Fuzz(func(t *testing.T, v string) {
		got := l.boundedValidator("etag", v)
		if got != "" && got != v {
			t.Errorf("boundedValidator(%q) = %q, want the input unchanged or empty", v, got)
		}
		if len(got) > maxValidatorBytes {
			t.Errorf("boundedValidator kept %d bytes, over the %d cap", len(got), maxValidatorBytes)
		}
		for i := range len(got) {
			if c := got[i]; (c < 0x20 && c != '\t') || c == 0x7f {
				t.Errorf("boundedValidator(%q) kept header-illegal byte %#x", v, c)
			}
		}
		if again := l.boundedValidator("etag", got); again != got {
			t.Errorf("boundedValidator not idempotent: %q -> %q", got, again)
		}
	})
}
