...
// and NewFeedWriter wire their own upstreams from the cfg they are given, so a test that
// exercises the Prowlarr proxy or the title harvest just passes its httptest client, and
// one that does not passes nil.
...
// loadingIndexer builds a server whose reload clock is RUNNING with its first load
// unresolved - the state start leaves behind while the loader is still working, and the
// one state in which readiness and the two producers' ORDER are observable at all.
// warmedIndexer cannot produce it: it loads synchronously and never marks the clock
// started, so every test built on it has loadPending() == false. That gap is why the
// readiness inversion (a published snapshot answering a fault because a load was in
// flight) and the install ordering (an older read landing after a newer publish) were
// both unreachable from the suite.
