...
// journalItem is one persisted RSS-journal record: the served wire item plus the
// journal bookkeeping the wire never carries.
...
// journalPass owns one pass's journal-scoped collaborators: the curation EVIDENCE this
// pass holds (see curationEvidence - a catalogue pass and a windowed pass hold
// different types, so the windowed one cannot be handed a catalogue-wide warned set),
// the loaded publication log, this pass's own publications, the entry-info lookup, the
// transition counters, and the pass clock.
type journalPass struct {
	w         *FeedWriter
	ev        curationEvidence
	published map[string]bool
	publish   map[string]bool
	// prior is the ownership fact the PREVIOUS snapshot holds (feedState.owners): the
	// only record of the votes of the owners this pass did NOT evaluate.
...
// records for one curated torrent: its tracker key (already scope-namespaced) and its
// info hash NAMESPACED BY SCOPE.
...
func baselinePublications(entries []seadex.Entry) map[string]bool {
	...
	for i := range entries {
		for j := range entries[i].Torrents {
			...
			if journalKey(t) == "" {
				// No journal key - an unsupported tail tracker, or a supported tracker
				// whose URL fails the ownership gate - so nothing is forfeited.
				continue
			}
...
// renderJournalItem materializes the journal item for key from its current curated
...
func (w *FeedWriter) renderJournalItem(key string, refs []curatedRef, infoFor EntryInfoFunc) (it journalItem, ok, noPasskey bool) {
	...
	// The order must be TOTAL, not just AniList-ascending: two occurrences of one
	// journal key can share an AniList id (a duplicated trs relation row on one record
	// - NOT two catalogue records sharing an alID, which
	// seadexapi.validatePageIdentities refuses outright at either scope), and a stable
	// sort then leaves catalogue order deciding which one is synthesized - the exact
	// dependency this sort exists to remove.
	...
	for _, occ := range ordered {
		...
		if !journalIdentityMatches(&it) {
			// Creation-time enforcement of the journal's GUID-to-Key invariant (the
			// carry gates and the reader's rebuild already enforce it): an occurrence
			// whose page URL is unpublishable (the publisher dropped it - e.g. a
			// bad-port-bearing URL that still passes trackerKey) would journal an item
		...
		if !validPersistedItem(&it) {
			...
			// amplify it well past the container memory budget (see
...
// AnimeBytes-passkey case out of plain unresolvability.
func (w *FeedWriter) journalLink(t *seadex.Torrent) (dl string, ok, linkless bool) {
	// Both arms read unusableABPasskey - the app's ONE home for "can this passkey build
	// a grabbable AnimeBytes link" (server.go, over internal/secretref), applied inside
	// downloadURLForScope for the first arm - so neither carries its own emptiness
	// test.
	scope := trackerScope(t.Tracker)
...
func (p *journalPass) carryItem(it *journalItem, scope string) (journalItem, bool) {
	...
	if scopeOfKey(it.Key) != scope {
		// The feed an item was loaded from IS its scope binding. A key naming the OTHER
		// tracker (only reachable from a corrupted or hand-edited snapshot) would
		// otherwise re-render with the wrong tracker's download link, consume a
		// rate-paced harvest query against the wrong upstream, and be dropped again by
		// every reader load (rebuild*DownloadURLs) for the item's whole journal window.
...
func (p *journalPass) prepareCarriedItem(it *journalItem) bool {
	...
	// canonical: decodeSnapshot runs normalizeSnapshotItems (validInfoHash) on both
	// feeds for BOTH consumers before loadPrevious returns them, which is where that
	// invariant is documented and pinned
	// (TestRebuildCanonicalizesStoredHashBeforeWarningRetraction).
...
// rebaseFutureFirstSeen applies the clock-skew correction both consumers of a persisted
// snapshot need: an item whose FirstSeen sits ahead of now (a snapshot restored from a
// future-skewed host, a hand-edited year-9999 value) has BOTH its journal timestamp and
// its derived PubDate reset to now, so its remaining lifetime is bounded by
// feedJournalMaxAge and the served <pubDate> can never advertise a negative release age
// (an arr delay profile would otherwise hold the release indefinitely).
...
func (p *journalPass) carryStoredItem(it *journalItem) (journalItem, bool) {
	// The journal's GUID-to-Key invariant, applied by the one arm that has both the
	// standing to refuse and no fresh render to self-heal from: a stored GUID that no
	// longer proves this item's journal identity (a cross-key, foreign-host, or empty
	// GUID from a hand-edited snapshot) must not be carried, because reload derives the
	// SERVED download link from that GUID, so a cross-key GUID would plant a fetch
	// target for a different torrent id on the same tracker for the item's whole
	// journal window.
	if !journalIdentityMatches(it) {
...
func (p *journalPass) refreshCarriedItem(it *journalItem, refs []curatedRef) (journalItem, bool) {
	...
	// releases by GUID, so a SeaDex URL-text change on the same tracker identity (a
	// query param appended, scheme/casing normalized) must never mint a new GUID and
	// re-trigger a grab for an already-journaled torrent.
	if journalIdentityMatches(it) {
		fresh.GUID = it.GUID
	}
	return fresh, true
...
// carryJournal re-renders or prunes one scope's previous journal items against this
// pass's evidence.
...
func (p *journalPass) journalIfNew(t *seadex.Torrent) (it journalItem, scope string, ok bool) {
	...
	if !ok {
		// The render failed for an upstream DATA reason on a tracker the operator has
		// ENABLED (no parseable title, an unpublishable page URL, a field over the
		// persisted limits).
		return journalItem{}, "", false
	}
	p.record(ids)
...
func (a titleAudit) served(key, title string) string {
	c, ok := a.census[key]
	if !ok {
		// This pass holds no file evidence for the key, so it is not entitled to judge
		// the title: serve what the cache holds, which is the title the app last SERVED
